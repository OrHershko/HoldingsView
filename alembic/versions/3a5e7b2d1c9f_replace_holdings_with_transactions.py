...
def upgrade() -> None:
    ...
    # Create the transaction_type ENUM only if it doesn't exist
    # Use raw SQL to check if the type exists first
    ...
    sa.Column('transaction_type', sa.Enum('BUY', 'SELL', name='transaction_type'), nullable=False),
...
def downgrade() -> None:
    ...
    # Drop the ENUM type only if no tables are using it
    connection = op.get_bind()
    result = connection.execute(sa.text(
        "SELECT 1 FROM information_schema.columns WHERE udt_name = 'transaction_type'"
    )).fetchone()
    
    if not result:
        # Only drop the ENUM if no columns are using it
        transaction_type_enum = postgresql.ENUM('BUY', 'SELL', name='transaction_type')
        transaction_type_enum.drop(connection)
    # ### end Alembic commands ###